...
<?php
	...
	if(!isset($_GET['code']) OR !isset($_GET['user'])){
		...
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM ecom_users WHERE activate_code=:code AND id=:id");
		...
		if($row['numrows'] > 0){
			if($row['status']){
				...
				try{
					$stmt = $conn->prepare("UPDATE ecom_users SET status=:status WHERE id=:id");

<?php
	...
	if(isset($_POST['reset'])){
		...
		if($password != $repassword){
			...
			$stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM ecom_users WHERE reset_code=:code AND id=:id");
			...
			if($row['numrows'] > 0){
				...
				try{
					$stmt = $conn->prepare("UPDATE ecom_users SET password=:password WHERE id=:id");

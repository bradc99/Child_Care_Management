...
<div class="content py-3">
    <div class="container-fluid">
        ...
        <div class="row row-cols-sm-1 row-cols-md-2 row-cols-xl-3 justify-content-center gx-3 gy-3">
            <?php 
            $babysitters = $conn->query("select * from users where type=3 and status=1");
            ...
            ?>
                <a class="col bs-item text-decoration-none text-dark" href="javascript:void(0)" data-id="<?= $row['id'] ?>">
                    ...
                    <div class="text-center">
                        <img id="bs-image" class="img-thumbnail bg-gradient-dark border-info" src="<?php echo validate_image(isset($row['id']) ? "uploads/{$row['id']}.png" : "").(isset($row['date_updated']) ? "?v=".strtotime($row['date_updated']) : "") ?>" alt="BS Image">
                    </div>
                    <h4 class="text-center"><b><?= ucwords($row['firstname']) ?></b></h4>
                    <div class="text-center"><small class="text-muted"><?= $row['username'] ?></small></div>
                    </div>
                </a>
            <?php endwhile; ?>
